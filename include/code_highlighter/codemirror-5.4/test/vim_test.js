...
testVim('dw_only_word', function(cm, vim, helpers) {
  // Test that there is only 1 word left, dw deletes till the end of the
  // line.
  cm.setCursor(0, 1);
  helpers.doKeys('d', 'w');
  eq(' ', cm.getValue());
...
testVim('dw_eol', function(cm, vim, helpers) {
  // Assert that dw does not delete the newline last word to delete is at end
  // of line.
  cm.setCursor(0, 1);
  helpers.doKeys('d', 'w');
  eq(' \nword2', cm.getValue());
...
testVim('dw_repeat', function(cm, vim, helpers) {
  // Assert that dw does delete newline it should go to the next line, and
...
testVim('yw_repeat', function(cm, vim, helpers) {
  // Assert that yw does yank newline it should go to the next line, and
  // that repeat works properly.
  var curStart = makeCursor(0, 1);
  cm.setCursor(curStart);
...
testVim('cw_repeat', function(cm, vim, helpers) {
  // Assert that cw does delete newline it should go to the next line, and
  // that repeat works properly.
  var curStart = makeCursor(0, 1);
  cm.setCursor(curStart);
  helpers.doKeys('c', '2', 'w');
...
testVim('search_register_escape', function(cm, vim, helpers) {
  // Check that the register is restored the user escapes rather than confirms.
