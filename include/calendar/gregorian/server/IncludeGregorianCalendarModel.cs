...
namespace elanat
{
    public class IncludeGregorianCalendarModel
    {
        ...
        public void SetValue(string CurrentDate = "")
        {
            ...
            // Set Mount Value
            lc.FillMountListItem(StaticObject.GetCurrentSiteGlobalLanguage());
            string CurrentMount = dt.Month.ToString();

            foreach (ListItem Mount in lc.MountListItem)
            {
            ...
            foreach (ListItem DayOfWeek in lc.DayOfWeakListItem)
            {
