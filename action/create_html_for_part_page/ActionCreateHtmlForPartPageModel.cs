...
namespace elanat
{
    public class ActionCreateHtmlForPartPageModel
    {
        private string CurrentLoadTag { get; set; } = "";
        private string CurrentHead { get; set; } = "";

        public void SetValue(string PageId, NameValueCollection QueryString)
        {
            // Set Name Value
            ...
            if (cc.Exist(CacheType, "el_alone_page_" + PageId + PageCacheKey))
            {
                ...
                HttpContext.Current.Response.Write(TmpAlonePageValue);
            ...
            {
                HttpContext.Current.Response.Write(Template.GetSiteTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("site_style_is_not_existed", StaticObject.GetCurrentSiteGlobalLanguage())));
                return;
            }

            if (!duss.SiteStyleActive.ZeroOneToBoolean())
            {
                HttpContext.Current.Response.Write(Template.GetSiteTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("site_style_is_inactive", StaticObject.GetCurrentSiteGlobalLanguage())));
                return;
            }
            ...
            HttpContext.Current.Response.Write(HtmlForPartPageStruct);
        }
    }
}