...
namespace elanat
{
    public class ActionEmailConfirmModel
    {
        public void SetValue(string UserNameOrUserEmail)
        {
            ...
            System.IO.Directory.CreateDirectory(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/elanat_system_data/confirm_email/user_" + UserId));
            System.IO.File.Create(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/elanat_system_data/confirm_email/user_" + UserId + "/confirm_email.ini")).Dispose();

            string[] Lines = new string[2];
            Lines[0] = "[email_code]";
            Lines[1] = sc.GetHash(RandomText);
            System.IO.File.WriteAllLines(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/elanat_system_data/confirm_email/user_" + UserId + "/confirm_email.ini"), Lines);
            ...
            GlobalClass.AlertStoredLanguageVariant("email_has_been_send_you_can_open_email_and_click_to_link_for_confirm_email", StaticObject.GetCurrentSiteGlobalLanguage());
        }
    }
}