...
namespace elanat
{
    public class ActionGetAttachmentListModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            if (!string.IsNullOrEmpty(QueryString["search"]))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/attachment/" + "App_Data/item_view/item_view.xml"), QueryString["search"].ToString().ProtectSqlInjection(true));
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp attachment_extension_name;", System.IO.Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString()).Remove(0, 1));
