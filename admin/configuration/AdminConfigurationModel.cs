...
namespace elanat
{
    public class AdminConfigurationModel
    {
        ...
        {
            string CurrentConfigurationPath = HttpContext.Current.Session["el_current_configuration_path"].ToString();

            if (System.IO.Path.GetExtension(CurrentConfigurationPath) == ".xml")
            {
                XmlDocument doc = new XmlDocument();
                doc.InnerXml = ConfigValue;
                doc.Save(HttpContext.Current.Server.MapPath(CurrentConfigurationPath));
            }
            else
            {
                string[] ConfigText = ConfigValue.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
                System.IO.File.WriteAllLines(HttpContext.Current.Server.MapPath(CurrentConfigurationPath), ConfigText);
            }
