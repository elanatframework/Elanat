...
namespace elanat
{
    public class ActionGetPageListModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            if (!string.IsNullOrEmpty(QueryString["search"]))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/page/" + "App_Data/item_view/item_view.xml"), QueryString["search"].ToString().ProtectSqlInjection(true));
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp page_extension_name;", System.IO.Path.GetExtension(dbdr.dr["page_physical_path"].ToString()).Remove(0, 1));
