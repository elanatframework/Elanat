...
namespace elanat
{
    public class ActionGetComponentListModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            List<string> ItemList = GlobalClass.GetItemViewList(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/component/" + "App_Data/item_view/item_view.xml"));
            ...
            if (!string.IsNullOrEmpty(QueryString["search"]))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/component/" + "App_Data/item_view/item_view.xml"), QueryString["search"].ToString().ProtectSqlInjection(true));
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp component_extension_name;", System.IO.Path.GetExtension(dbdr.dr["component_physical_path"].ToString()).Remove(0, 1));
