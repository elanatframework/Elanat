...
namespace elanat
{
    public class ActionGetPluginListModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            if (!string.IsNullOrEmpty(QueryString["search"]))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/plugin/" + "App_Data/item_view/item_view.xml"), QueryString["search"].ToString().ProtectSqlInjection(true));
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp plugin_extension_name;", System.IO.Path.GetExtension(dbdr.dr["plugin_physical_path"].ToString()).Remove(0, 1));
