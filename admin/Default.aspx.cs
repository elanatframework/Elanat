    ...
    public partial class Admin : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            foreach (string NameValue in Request.QueryString)
                QueryStringCollection.Add(NameValue, Request.QueryString[NameValue].ToString());
            ...
            foreach (string key in (StaticObject.AdminMainCacheParameters.Split(',')))
                if (!string.IsNullOrEmpty(QueryStringCollection[key]))
                    AdminMainCacheKey += ":" + QueryStringCollection[key].ToString();
            ...
            {
                CurrentLanguageGlobalName = Request.QueryString["language"].ProtectSqlInjection();
                ...
                {
                    Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("language_is_not_existed", CurrentLanguageGlobalName)));
                    return;
                }

                if (!dul.LanguageActive.ZeroOneToBoolean())
                {
                    Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("language_is_inactive", CurrentLanguageGlobalName)));
                    return;
                }
            ...
            // Set Site Value
            CurrentSiteGlobalName = (!string.IsNullOrEmpty(Request.QueryString["site"])) ? Request.QueryString["site"].ProtectSqlInjection() : StaticObject.ConfigDocument.SelectSingleNode("elanat_root/properties/default_site").Attributes["value"].Value;
            ...
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("site_is_not_existed", CurrentLanguageGlobalName)));
                return;
            }

            if (!dus.SiteActive.ZeroOneToBoolean())
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("site_is_inactive", CurrentLanguageGlobalName)));
                return;
            }
            ...
            {
                CurrentLanguageGlobalName = (!string.IsNullOrEmpty(Request.QueryString["language"])) ? Request.QueryString["language"].ProtectSqlInjection() : StaticObject.DefaultAdminLanguage;
            ...
            if (StaticObject.GetCurrentUserId() != "0")
            {
                ...
                {
                    Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("user_is_not_existed", CurrentLanguageGlobalName)));
                    return;
                }

                if (!duu.UserActive.ZeroOneToBoolean())
                {
                    Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("user_is_inactive", CurrentLanguageGlobalName)));
                    return;
                }
            ...
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("admin_template_is_not_existed", CurrentLanguageGlobalName)));
                return;
            }

            if (!duat.AdminTemplateActive.ZeroOneToBoolean())
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("admin_template_is_inactive", CurrentLanguageGlobalName)));
                return;
            }
            ...
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("admin_style_is_not_existed", CurrentLanguageGlobalName)));
                return;
            }

            if (!duas.AdminStyleActive.ZeroOneToBoolean())
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("admin_style_is_inactive", CurrentLanguageGlobalName)));
                return;
            }
            ...
            // Set Component Value
            CurrentComponentName = (!string.IsNullOrEmpty(Request.QueryString["component_name"])) ? Request.QueryString["component_name"].ProtectSqlInjection() : ElanatComponent;
            ...
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("component_is_not_existed", CurrentLanguageGlobalName)));
                return;
            }

            if (!duc.ComponentActive.ZeroOneToBoolean())
            {
                Response.Write(Template.GetAdminTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("component_is_inactive", CurrentLanguageGlobalName)));
                return;
            }

            // Load Component Catalog
            XmlDocument ComponentCatalogDocument = new XmlDocument();
            ComponentCatalogDocument.Load(Server.MapPath(StaticObject.AdminPath + "/" + duc.ComponentDirectoryPath + "/catalog.xml"));

            CurrentStaticHead += ComponentCatalogDocument.SelectSingleNode("component_catalog_root/component_static_head").InnerXml.Replace("<![CDATA[", "").Replace("]]>", "");

            CurrentLoadTag += ComponentCatalogDocument.SelectSingleNode("component_catalog_root/component_load_tag").InnerXml.Replace("<![CDATA[", "").Replace("]]>", "");
            ...
            // Turn On Http Only For Secure Attach
            Request.Cookies["ASP.NET_SessionId"].HttpOnly = (ElanatConfig.GetNode("security/use_http_only_for_session_id").Attributes["active"].Value == "true");
            DateTime DateTimeSessionLifeTime = DateTime.Now.AddSeconds(double.Parse(ElanatConfig.GetNode("security/session_life_time").Attributes["value"].Value));
            Request.Cookies["ASP.NET_SessionId"].Expires = DateTimeSessionLifeTime;
        }
    }
}