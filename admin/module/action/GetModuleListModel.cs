...
namespace elanat
{
    public class ActionGetModuleListModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            List<string> ItemList = GlobalClass.GetItemViewList(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/module/" + "App_Data/item_view/item_view.xml"));
            ...
            if (!string.IsNullOrEmpty(QueryString["search"]))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/module/" + "App_Data/item_view/item_view.xml"), QueryString["search"].ToString().ProtectSqlInjection(true));
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp module_extension_name;", System.IO.Path.GetExtension(dbdr.dr["module_physical_path"].ToString()).Remove(0, 1));
