...
namespace elanat
{
    public class ActionGetFileDirectoryListModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            // Set Current Path
            string CurrentPhysicalPath = (!string.IsNullOrEmpty(QueryString["directory"])) ? HttpContext.Current.Server.MapPath(QueryString["directory"].ToString()) : HttpContext.Current.Server.MapPath(StaticObject.SitePath + "");
            string CurrentPath = (!string.IsNullOrEmpty(QueryString["directory"])) ? QueryString["directory"].ToString() : "";
            ...
            List<string> ItemList = GlobalClass.GetItemViewList(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/file_manager/" + "App_Data/item_view/item_view.xml"));
            ...
            {
                UseSearch = true;
                SearchValue = QueryString["search"].ToString();
            }
            ...
            if (!string.IsNullOrEmpty(QueryString["search"]))
            {
                List<string> SearchedItemList = GlobalClass.GetSearchedItemList(HttpContext.Current.Server.MapPath(StaticObject.AdminPath + "/file_manager/" + "App_Data/item_view/item_view.xml"));
            ...
            {
                string ColumnName = QueryString["column_name"].ToString();

                bool IsDescSort = false;
                if (!string.IsNullOrEmpty(QueryString["is_desc"]))
                    IsDescSort = (QueryString["is_desc"] == "true");
            ...
            foreach (FileInfo file in fileInfo)
            {
                ...
                foreach (string Text in ItemList)
                {
                    ...
                    string FileExtension = (FileAndDirectory.IsPublicExtension(file.Name)) ? System.IO.Path.GetExtension(file.Name).Remove(0, 1) : "blank";
