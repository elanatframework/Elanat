...
namespace elanat
{
    public class SiteRssWriterModel
    {
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            {
                HttpContext.Current.Response.Write(Language.GetLanguage("page_is_inactive", StaticObject.GetCurrentSiteGlobalLanguage()));
                return;
            }
            ...
            if (!string.IsNullOrEmpty(QueryString["site"]))
                CacheKey += ":" + QueryString["site"].ToString();

            if (!string.IsNullOrEmpty(QueryString["user_id"]))
                CacheKey += ":" + QueryString["user_id"].ToString();

            if (!string.IsNullOrEmpty(QueryString["category_name"]))
                CacheKey += ":" + QueryString["category_name"].ToString();

            if (!string.IsNullOrEmpty(QueryString["content_type"]))
                CacheKey += ":" + QueryString["content_type"].ToString();

            CacheClass cc = new CacheClass();

            if (cc.ExistInDisk(CacheKey))
            {
                HttpContext.Current.Response.Write(cc.GetValue("disk", CacheKey));
                return;
            }
            ...
            // Set Site Feed
            if (!string.IsNullOrEmpty(QueryString["site"]) && ActiveAuthorFeed)
                if (QueryString["site"].ToString().IsNumber())
                {
                    string SiteId = dus.GetSiteIdBySiteGlobalName(QueryString["site"].ToString().ProtectSqlInjection());

                    InnerAttach = "where el_site.site_id=" + SiteId;
                }

            // Set Content Type Feed
            if (!string.IsNullOrEmpty(QueryString["content_type"]) && ActiveContentTypeFeed)
            {
            ...
            // Set Author Feed
            if (!string.IsNullOrEmpty(QueryString["user_id"]) && ActiveAuthorFeed)
                if (QueryString["user_id"].IsNumber())
                {
                    string Author = QueryString["user_id"].ToString().ProtectSqlInjection();
            ...
            // Set Category Feed
            if (!string.IsNullOrEmpty(QueryString["category_name"]) && ActiveCategoryFeed)
            {
                string CategoryName = QueryString["category_name"].ToString().ProtectSqlInjection();
            ...
            HttpContext.Current.Response.Write(RssContent);
