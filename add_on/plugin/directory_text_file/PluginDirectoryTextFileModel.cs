...
namespace elanat
{
    public class PluginDirectoryTextFileModel
    {
        ...
        public void SetValue(NameValueCollection QueryString, NameValueCollection FormData)
        {
            // Set Language
            ...
            if (!string.IsNullOrEmpty(QueryString["file_path"]))
            {
                FilePathValue = QueryString["file_path"];
                DirectoryPathValue = QueryString["current_add_on_path"];
                ...
                FileInfo file = new FileInfo(HttpContext.Current.Server.MapPath(StaticObject.SitePath + FilePathValue));
            ...
            if (!string.IsNullOrEmpty(QueryString["directory_path"]))
            {
                DirectoryPathValue = QueryString["directory_path"];

                ShowFileListModeClass = ShowFileListModeClass.DeleteHtmlClass("el_hidden");

                DirectoryInfo DirInfo = new DirectoryInfo(HttpContext.Current.Server.MapPath(StaticObject.SitePath + DirectoryPathValue));
                ...
                foreach (FileInfo file in Directories)
                {
                    ...
                    string DirectoryFullPath = HttpContext.Current.Server.MapPath(StaticObject.SitePath + DirectoryPathValue);
        ...
        public void SaveFile()
        {
            ...
            if (Path.GetExtension(FilePathValue) == ".xml")
            {
                ...
                doc.Save(HttpContext.Current.Server.MapPath(FilePathValue));
            }
            else
            {
                string[] FileText = FileTextValue.Trim().Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
                System.IO.File.WriteAllLines(HttpContext.Current.Server.MapPath(FilePathValue), FileText);
            }
        ...
        public void Return()
        {
            HttpContext.Current.Response.Redirect("?directory_path=" + DirectoryPathValue);
        }

        public void SuccessView()
        {
            HttpContext.Current.Response.Redirect(StaticObject.SitePath + "add_on/plugin/directory_text_file/action/SuccessMessage.aspx", true);
        }
    }
}