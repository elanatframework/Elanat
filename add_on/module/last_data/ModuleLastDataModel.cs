...
namespace elanat
{
    public class ModuleLastDataModel
    {
            ...
            if (node["last_attachment"].Attributes["active"].InnerText == "true")
            {
                ...
                if (dbdr.dr != null && dbdr.dr.HasRows)
                    while (dbdr.dr.Read())
                    {
                        ...
                        if (FileAndDirectory.IsImageExtension(System.IO.Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString())))
                        ...
                        TmpListItemTemplate = TmpListItemTemplate.Replace("$_asp attachment_extension;", System.IO.Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString()).Remove(0, 1));
