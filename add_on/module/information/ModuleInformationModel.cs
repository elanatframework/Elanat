...
namespace elanat
{
    public class ModuleInformationModel
    {
        ...
        public void SetValue()
        {
            ...
            InformationOptionDocument.Load(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "add_on/module/information/option/information_option.xml"));
            ...
            if (node["log_error"].Attributes["active"].Value == "true")
            {
                ...
                BoxTemplate = BoxTemplate.Replace("$_asp number_of_log_error_value;", fad.GetDirectoryFilesCount(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/logs")).ToString());
            ...
            if (node["upload_size"].Attributes["active"].Value == "true")
            {
                ...
                FileAndDirectory fad = new FileAndDirectory();
                BoxTemplate = BoxTemplate.Replace("$_asp upload_directory_size_value;", fad.GetDirectoryFileSize(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload"), false).ToBitSizeTuning());
            ...
            if (node["tmp_size"].Attributes["active"].Value == "true")
            {
                ...
                FileAndDirectory fad = new FileAndDirectory();
                BoxTemplate = BoxTemplate.Replace("$_asp tmp_directory_size_value;", fad.GetDirectoryFileSize(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp"), false).ToBitSizeTuning());
