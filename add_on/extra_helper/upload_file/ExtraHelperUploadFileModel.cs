...
namespace elanat
{
    public class ExtraHelperUploadFileModel
    {
        ...
        public HttpPostedFile FilePathUploadValue { get; set; }
        ...
        public void StartUpload()
        {
            ...
            if (UseFilePathValue)
            {
                ...
                DirectoryName = FileAndDirectory.GetNewDirectoryNameIfDirectoryExist(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp/"), Path.GetFileNameWithoutExtension(TmpFilePhysicalName));

                System.IO.Directory.CreateDirectory(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp/" + DirectoryName));
                webClient.DownloadFile(FilePathTextValue, HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp/" + DirectoryName + "/" + TmpFilePhysicalName));
            }
            else
            {
                ...
                System.IO.Directory.CreateDirectory(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp/" + DirectoryName));
                FilePathUploadValue.SaveAs(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp/" + DirectoryName + "/" + TmpFilePhysicalName));
            }

            ev.FilePhysicalName = TmpFilePhysicalName;
            string FilePhysicalName = ev.GetUploadFileValue();

            if (!Directory.Exists(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload/" + FileDirectoryPath)))
                Directory.CreateDirectory(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload/" + FileDirectoryPath));

            FilePhysicalName = FileAndDirectory.GetNewFileNameIfFileNameExist(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload/" + FileDirectoryPath), FilePhysicalName);


            // Move Upload File In "upload" Directory To Upload
            File.Move(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/tmp/" + DirectoryName + "/" + TmpFilePhysicalName), HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload/" + FileDirectoryPath + "/" + FilePhysicalName));
            ...
            if (FileAndDirectory.IsImageExtension(FileExtension))
                ...
                {
                    if (!System.IO.Directory.Exists(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload/" + FileDirectoryPath + "/thumb/")))
                        System.IO.Directory.CreateDirectory(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "upload/" + FileDirectoryPath + "/thumb/"));
