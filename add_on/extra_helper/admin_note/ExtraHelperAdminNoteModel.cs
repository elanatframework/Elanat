...
namespace elanat
{
    public class ExtraHelperAdminNoteModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            ColumnListDocument.Load(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "add_on/extra_helper/admin_note/App_Data/column_list/column_list.xml"));
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowListItemTemplate = TmpRowListItemTemplate.Replace("$_asp content_avatar_path;", (!string.IsNullOrEmpty(dbdr.dr["content_avatar_physical_name"].ToString())) ? HttpContext.Current.Server.MapPath(StaticObject.SitePath + "client/image/content_avatar/" + dbdr.dr["content_avatar_physical_name"].ToString()) : "");
                    TmpRowListItemTemplate = TmpRowListItemTemplate.Replace("$_asp content_icon_path;", (!string.IsNullOrEmpty(dbdr.dr["content_icon_physical_name"].ToString())) ? HttpContext.Current.Server.MapPath(StaticObject.SitePath + "client/image/content_icon/" + dbdr.dr["content_icon_physical_name"].ToString()) : "");
        ...
        public string GetAttachment(string ContentId)
        {
            ...
            while (dbdr.dr.Read())
            {
                ...
                else
                {
                    ...
                    TmpAttachmentTemplateListItem = TmpAttachmentTemplateListItem.Replace("$_asp attachment_extension_icon;", System.IO.Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString()).Remove(0, 1));
