...
namespace elanat
{
    public class SiteCommentModel
    {
        ...
        public void SendComment()
        {
            ...
            if (UseUploadPathValue || UploadPathUploadValue.HtmlInputHasFile())
            {
                ...
                if (FileSize > int.Parse(MaxOfUploadSizeUpload))
                {
                    ...
                    GlobalClass.Alert(Language.GetLanguage("file_size_must_be_less_than_asp", StaticObject.GetCurrentAdminGlobalLanguage()).Replace("$_asp max_of_file_size_upload;", long.Parse(MaxOfUploadSizeUpload).ToBitSizeTuning()), "problem");
                }
            }
        ...
        {
            HttpContext.Current.Response.Redirect(StaticObject.SitePath + "page/comment/action/SuccessMessage.aspx?use_retrieved=true");
        }
    }
}