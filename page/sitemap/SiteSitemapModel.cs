...
namespace elanat
{
    public class SiteSitemapModel
    {
        public void SetValue()
        {
            // Set Value
            XmlDocument SitemapOptionDocument = new XmlDocument();
            SitemapOptionDocument.Load(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "add_on/extra_helper/sitemap_option/option/sitemap_option.xml"));
            ...
            if (ActiveContentType)
            {
                ...
                lc.FillContentTypeListItem(StaticObject.GetCurrentSiteGlobalLanguage());
            ...
            if (ActiveCategory)
            {
                ...
                // Set Category Item
                ListClass lc = new ListClass();
                lc.FillCategoryListItemTree(StaticObject.GetCurrentSiteSiteId(), SpaceTemplate);
                ...
                foreach (ListItem item in lc.CategoryListItemTree)
                {
                    ...
                    TmpSiteMapCategoryListItemTemplate = TmpSiteMapCategoryListItemTemplate.Replace("$_asp category_name;", lc.CategoryListItemOnlySpace[CategoryIndexer].Text + Language.GetHandheldLanguage(lc.CategoryListItemTreeWithoutSpace[CategoryIndexer].Text, StaticObject.GetCurrentSiteGlobalLanguage()));
            ...
            if (ActiveLanguage)
            {
                ...
                dbdr.dr = db3.GetProcedure("get_language_name_and_global_name_list");
            ...
            HttpContext.Current.Response.Write(SiteMapBoxTemplate.Replace("$_asp item;", SumSiteMapListItemTemplate));
        }
    }
}