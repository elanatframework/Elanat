...
namespace elanat
{
    public class ActionSearchModel
    {
        ...
        public void SetValue(NameValueCollection QueryString)
        {
            ...
            if (MnimumSearchCharacter > QueryString["search"].ToString().Length)
            ...
            string TitleOrText = "";
            string Where = "";
            string Search = QueryString["search"].ToString().Trim().ProtectSqlInjection(true);

            int PageIndex = 1;
            if (!string.IsNullOrEmpty(QueryString["page"]))
                PageIndex = int.Parse(QueryString["page"].ToString());
            ...
            if (!string.IsNullOrEmpty(QueryString["type"]))
            {
                switch (QueryString["type"].ToString())
                {
            ...
            if (!string.IsNullOrEmpty(QueryString["from_date"]))
                Where += " and el_content.content_date_create >= '" + QueryString["from_date"].ToString().Replace("-", "/").ProtectSqlInjection() + "'";


            if (!string.IsNullOrEmpty(QueryString["until_date"]))
                Where += " and el_content.content_date_create <= '" + QueryString["until_date"].ToString().Replace("-", "/").ProtectSqlInjection() + "'";


            if (!string.IsNullOrEmpty(QueryString["category"]))
            {
                if (QueryString["category"] != "all")
                    if (QueryString["category"].IsNumber())
                        Where = " and el_content.category_id=" + QueryString["category"].ToString().ProtectSqlInjection();
            }
            ...
            {
                int ParsedValue;
                if (int.TryParse(QueryString["page"], out ParsedValue))
            ...
            if (!string.IsNullOrEmpty(QueryString["category"]))
                CategorySelectedIndex = "&category=" + QueryString["category"];

            string FromDate = "";
            if (!string.IsNullOrEmpty(QueryString["from_date"]))
                FromDate = "&from_date=" + QueryString["from_date"];

            string UntilDate = "";
            if (!string.IsNullOrEmpty(QueryString["until_date"]))
                UntilDate = "&until_date=" + QueryString["until_date"];
