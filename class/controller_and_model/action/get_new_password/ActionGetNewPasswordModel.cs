...
namespace Elanat
{
    public partial class ActionGetNewPasswordModel : CodeBehindModel
    {
        public void SetValue(HttpContext context, string UserNameOrUserEmail)
        {
            ...
            Directory.CreateDirectory(StaticObject.ServerMapPath(StaticObject.SitePath + "App_Data/elanat_system_data/new_password_code/user_" + UserId));
            File.Create(StaticObject.ServerMapPath(StaticObject.SitePath + "App_Data/elanat_system_data/new_password_code/user_" + UserId + "/new_password_code.ini")).Dispose();

            string[] Lines = new string[2];
            Lines[0] = "[password_code]";
            Lines[1] = sc.GetHash(RandomText);
            File.WriteAllLines(StaticObject.ServerMapPath(StaticObject.SitePath + "App_Data/elanat_system_data/new_password_code/user_" + UserId + "/new_password_code.ini"), Lines);
            ...
            Write(GlobalClass.AlertStoredLanguageVariant("email_has_been_send_you_can_open_email_and_click_to_link_for_show_new_password", StaticObject.GetCurrentSiteGlobalLanguage()));
        }
    }
}