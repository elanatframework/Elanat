...
namespace Elanat
{
    public partial class ActionCreateHtmlForPartPageModel : CodeBehindModel
    {
        private string CurrentLoadTag { get; set; } = "";
        private string CurrentHead { get; set; } = "";

        public void SetValue(HttpContext context, string PageId, List<ListItem> QueryString)
        {
            // Set Cache Type
            ...
            foreach (string key in (dup.PageCacheParameters.Split(',')))
                if (!string.IsNullOrEmpty(QueryString.GetValue(key)))
                    PageCacheKey += ":" + QueryString.GetValue(key).ToString();
            ...
            {
                Write(Template.GetSiteTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("site_style_is_not_existed", StaticObject.GetCurrentSiteGlobalLanguage())));
                return;
            }

            if (!duss.SiteStyleActive.ZeroOneToBoolean())
            {
                Write(Template.GetSiteTemplate("part/role_access/view").Replace("$_asp inaccess_reason;", Language.GetLanguage("site_style_is_inactive", StaticObject.GetCurrentSiteGlobalLanguage())));
                return;
            }
