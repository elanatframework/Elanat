...
namespace Elanat
{
    public partial class ActionGetExtraHelperListModel : CodeBehindModel
    {
        ...
        public void SetValue(List<ListItem> QueryString)
        {
            ...
            string ClientLanguageVariantTemplate = doc.SelectSingleNode("template_root/table/extra_helper/client_language_variant").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderTemplate = doc.SelectSingleNode("template_root/table/extra_helper/header/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderListItemTemplate = doc.SelectSingleNode("template_root/table/extra_helper/header/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowBoxTemplate = doc.SelectSingleNode("template_root/table/extra_helper/row/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowListItemTemplate = doc.SelectSingleNode("template_root/table/extra_helper/row/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            ...
            if (!string.IsNullOrEmpty(QueryString.GetValue("search")))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(StaticObject.ServerMapPath(StaticObject.AdminPath + "/extra_helper/" + "App_Data/item_view/item_view.xml"), QueryString.GetValue("search").ProtectSqlInjection(true));
            ...
            dbdr.dr = db.GetProcedure("get_extra_helper", ParametersNameList, ParametersValueList);
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    XmlNode ItemNode = doc.SelectSingleNode("template_root/table/extra_helper/item");
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp extra_helper_id;", dbdr.dr["extra_helper_id"].ToString());
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp extra_helper_directory_path;", dbdr.dr["extra_helper_directory_path"].ToString());
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp extra_helper_extension_name;", Path.GetExtension(dbdr.dr["extra_helper_physical_path"].ToString()).Remove(0, 1));
                    TmpRowBoxTemplate = (bool)dbdr.dr["extra_helper_active"] ? TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "active").Replace("$_asp check_Active_Inactive;", "Inactive") : TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "inactive").Replace("$_asp check_Active_Inactive;", "Active");
            ...
            // Get Extra Helper Count
            DataUse.ExtraHelper dueh = new DataUse.ExtraHelper();
            string RowCount = dueh.GetExtraHelperCountByAttach(Where, TmpWhere);
            ...
            ListValue = ClientLanguageVariantTemplate + HeaderTemplate + SumRowBoxTemplate + InnerModuleClass.PageNumbers(int.Parse(RowCount), StaticObject.AdminPath + "/extra_helper/action/GetExtraHelperList.aspx" + Query, CurrentPage, 10, "?page=", true, "div_TableBox");
        }
    }
}