...
namespace Elanat
{
    public partial class ActionGetFootPrintListModel : CodeBehindModel
    {
        ...
        public void SetValue(List<ListItem> QueryString)
        {
            ...
            List<string> ItemList = GlobalClass.GetItemViewList(StaticObject.ServerMapPath(StaticObject.AdminPath + "/foot_print/" + "App_Data/item_view/item_view.xml"));
            ...
            if (!string.IsNullOrEmpty(QueryString.GetValue("search")))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(StaticObject.ServerMapPath(StaticObject.AdminPath + "/foot_print/" + "App_Data/item_view/item_view.xml"), QueryString.GetValue("search").ProtectSqlInjection(true));
            ...
            ListValue = ClientLanguageVariantTemplate + HeaderTemplate + SumRowBoxTemplate + InnerModuleClass.PageNumbers(int.Parse(RowCount), StaticObject.AdminPath + "/foot_print/action/GetFootPrintList.aspx" + Query, CurrentPage, 10, "?page=", true, "div_TableBox");
        }
    }
}