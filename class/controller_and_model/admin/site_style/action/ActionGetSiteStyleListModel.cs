...
namespace Elanat
{
    public partial class ActionGetSiteStyleListModel : CodeBehindModel
    {
        ...
        public void SetValue(List<ListItem> QueryString)
        {
            ...
            string ClientLanguageVariantTemplate = doc.SelectSingleNode("template_root/table/site_style/client_language_variant").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderTemplate = doc.SelectSingleNode("template_root/table/site_style/header/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderListItemTemplate = doc.SelectSingleNode("template_root/table/site_style/header/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowBoxTemplate = doc.SelectSingleNode("template_root/table/site_style/row/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowListItemTemplate = doc.SelectSingleNode("template_root/table/site_style/row/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            ...
            if (!string.IsNullOrEmpty(QueryString.GetValue("search")))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(StaticObject.ServerMapPath(StaticObject.AdminPath + "/site_style/" + "App_Data/item_view/item_view.xml"), QueryString.GetValue("search").ProtectSqlInjection(true));
            ...
            dbdr.dr = db.GetProcedure("get_site_style", ParametersNameList, ParametersValueList);
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp site_style_id;", dbdr.dr["site_style_id"].ToString());
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp site_style_extension_name;", Path.GetExtension(dbdr.dr["site_style_physical_path"].ToString()).Remove(0, 1));
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp site_style_directory_path;", dbdr.dr["site_style_directory_path"].ToString());
                    TmpRowBoxTemplate = (bool)dbdr.dr["site_style_active"] ? TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "active").Replace("$_asp check_Active_Inactive;", "Inactive") : TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "inactive").Replace("$_asp check_Active_Inactive;", "Active");
            ...
            // Get Site Style Count
            DataUse.SiteStyle duss = new DataUse.SiteStyle();
            string RowCount = duss.GetSiteStyleCountByAttach(Where, TmpWhere);
            ...
            ListValue = ClientLanguageVariantTemplate + HeaderTemplate + SumRowBoxTemplate + InnerModuleClass.PageNumbers(int.Parse(RowCount), StaticObject.AdminPath + "/site_style/action/GetSiteStyleList.aspx" + Query, CurrentPage, 10, "?page=", true, "div_TableBox");
        }
    }
}