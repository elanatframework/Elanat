...
namespace Elanat
{
    public partial class ActionGetContentReplyListModel : CodeBehindModel
    {
        ...
        public void SetValue(List<ListItem> QueryString)
        {
            ...
            string ClientLanguageVariantTemplate = doc.SelectSingleNode("template_root/table/content_reply/client_language_variant").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderTemplate = doc.SelectSingleNode("template_root/table/content_reply/header/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderListItemTemplate = doc.SelectSingleNode("template_root/table/content_reply/header/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowBoxTemplate = doc.SelectSingleNode("template_root/table/content_reply/row/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowListItemTemplate = doc.SelectSingleNode("template_root/table/content_reply/row/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            ...
            List<string> ItemList = GlobalClass.GetItemViewList(StaticObject.ServerMapPath(StaticObject.AdminPath + "/content_reply/" + "App_Data/item_view/item_view.xml"));
            ...
            if (!string.IsNullOrEmpty(QueryString.GetValue("search")))
            {
                Where += GlobalClass.GetSearchedItemListWhereQuery(StaticObject.ServerMapPath(StaticObject.AdminPath + "/content_reply/" + "App_Data/item_view/item_view.xml"), QueryString.GetValue("search").ProtectSqlInjection(true), true);
            ...
            dbdr.dr = db.GetProcedure("get_content_reply", ParametersNameList, ParametersValueList);
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
                while (dbdr.dr.Read())
                {
                    ...
                    XmlNode ItemNode = doc.SelectSingleNode("template_root/table/content_reply/item");
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp content_reply_id;", dbdr.dr["content_reply_id"].ToString());
                    TmpRowBoxTemplate = (bool)dbdr.dr["content_reply_active"] ? TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "active").Replace("$_asp check_Active_Inactive;", "Inactive") : TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "inactive").Replace("$_asp check_Active_Inactive;", "Active");
            ...
            // Get Content Reply Count
            DataUse.ContentReply ducr = new DataUse.ContentReply();
            string RowCount = ducr.GetContentReplyCountByAttach(Where, TmpWhere);
            ...
            ListValue = ClientLanguageVariantTemplate + HeaderTemplate + SumRowBoxTemplate + InnerModuleClass.PageNumbers(int.Parse(RowCount), StaticObject.AdminPath + "/content_reply/action/GetContentReplyList.aspx" + Query, CurrentPage, 10, "?page=", true, "div_TableBox");
        }
    }
}