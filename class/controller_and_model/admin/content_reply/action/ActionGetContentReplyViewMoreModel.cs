...
namespace Elanat
{
    public partial class ActionGetContentReplyViewMoreModel : CodeBehindModel
    {
        public string GetViewMore(string CommentReplyId)
        {
			...
			dbdr.dr = db.GetProcedure("get_current_content_reply", "@content_reply_id", CommentReplyId);

            if (dbdr.dr != null && dbdr.dr.HasRows)
            {
                dbdr.dr.Read();
                List<string> ViewMoreList = GlobalClass.GetItemViewMoreList(StaticObject.AdminPath + "/content_reply/");
