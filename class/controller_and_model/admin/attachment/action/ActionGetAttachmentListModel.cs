...
namespace Elanat
{
    public partial class ActionGetAttachmentListModel : CodeBehindModel
    {
        ...
        public void SetValue(List<ListItem> QueryString)
        {
            ...
            string ClientLanguageVariantTemplate = doc.SelectSingleNode("template_root/table/attachment/client_language_variant").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderTemplate = doc.SelectSingleNode("template_root/table/attachment/header/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string HeaderListItemTemplate = doc.SelectSingleNode("template_root/table/attachment/header/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowBoxTemplate = doc.SelectSingleNode("template_root/table/attachment/row/box").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            string RowListItemTemplate = doc.SelectSingleNode("template_root/table/attachment/row/list_item").InnerTextAfterSetNodeVariant(StaticObject.GetCurrentAdminGlobalLanguage());
            ...
            if (!string.IsNullOrEmpty(QueryString.GetValue("search")))
            {
                Where = GlobalClass.GetSearchedItemListWhereQuery(StaticObject.ServerMapPath(StaticObject.AdminPath + "/attachment/" + "App_Data/item_view/item_view.xml"), QueryString.GetValue("search").ProtectSqlInjection(true));
            ...
            dbdr.dr = db.GetProcedure("get_attachment", ParametersNameList, ParametersValueList);
                ...
                while (dbdr.dr.Read())
                {
                    ...
                    foreach (string Text in ItemList)
                    {
                        ...
                        if (Text == "attachment_size")
                            TmpItemBoxTemplate = TmpItemBoxTemplate.Replace("$_db attachment_size;", long.Parse(dbdr.dr["attachment_size"].ToString()).ToBitSizeTuning());

                        TmpItemBoxTemplate = TmpItemBoxTemplate.Replace("$_db " + Text + ";", dbdr.dr[Text].ToString());
                    ...
                    foreach (XmlNode node in ItemNode.ChildNodes)
                        if (node.Attributes["active"] != null)
                            if (node.Attributes["active"].Value.TrueFalseToBoolean())
                            {
                                ...
                                {
                                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_db attachment_size;", long.Parse(dbdr.dr["attachment_size"].ToString()).ToBitSizeTuning());
                                    continue;
                                }
                    ...
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp attachment_id;", dbdr.dr["attachment_id"].ToString());
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp attachment_directory_path;", dbdr.dr["attachment_directory_path"].ToString());
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp attachment_physical_name;", dbdr.dr["attachment_physical_name"].ToString());
                    TmpRowBoxTemplate = TmpRowBoxTemplate.Replace("$_asp attachment_extension_name;", Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString()).Remove(0, 1));
                    TmpRowBoxTemplate = (bool)dbdr.dr["attachment_active"] ? TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "active").Replace("$_asp check_Active_Inactive;", "Inactive") : TmpRowBoxTemplate.Replace("$_asp check_active_inactive;", "inactive").Replace("$_asp check_Active_Inactive;", "Active");
            ...
            string RowCount = dua.GetAttachmentCountByAttach(Where, TmpWhere);
            ...
            ListValue = ClientLanguageVariantTemplate + HeaderTemplate + SumRowBoxTemplate + InnerModuleClass.PageNumbers(int.Parse(RowCount), StaticObject.AdminPath + "/attachment/action/GetAttachmentList.aspx" + Query, CurrentPage, 10, "?page=", true, "div_TableBox");
        }
    }
}