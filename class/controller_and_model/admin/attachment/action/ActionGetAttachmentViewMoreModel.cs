...
namespace Elanat
{
    public partial class ActionGetAttachmentViewMoreModel : CodeBehindModel
    {
        public string GetViewMore(string AttachmentId)
        {
            ...
            {
                dbdr.dr.Read();
                List<string> ViewMoreList = GlobalClass.GetItemViewMoreList(StaticObject.AdminPath + "/attachment/");
                ...
                foreach (string Text in ViewMoreList)
                {
                    ...
                    if (Text == "attachment_size")
                        TmpViewMoreItem = TmpViewMoreItem.Replace("$_asp item_value;", long.Parse(dbdr.dr["attachment_size"].ToString()).ToBitSizeTuning());
