...
namespace elanat
{
    public class CategoryMap
    {
        ...
        {
            XmlTextWriter writer = new XmlTextWriter(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "App_Data/elanat_system_data/site_data/" + SiteGlobalName + "/category_map/category_map.xml"), System.Text.Encoding.UTF8);
            writer.WriteStartDocument(true);
            writer.Formatting = Formatting.Indented;

            // Set Current Value
            ListClass li = new ListClass();
            li.FillCategoryListItemTree(SiteId, "$");


            ListItem[] CategoryList = li.CategoryListItemTree;

            int i_CategoryListCount = CategoryList.Length;
