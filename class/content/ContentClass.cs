    ...
    public class ContentClass
    {
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
            {
                while (dbdr.dr.Read())
                {
                    ...
                    else
                    {
                        ...
                        TmpAttachmentFileTemplate = TmpAttachmentFileTemplate.Replace("$_asp attachment_extension_icon;", System.IO.Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString()).Remove(0, 1));
        ...
        public string GetContent(string GroupId, string Where = null, int PageNumber = 0)
        {
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
            {
                while (dbdr.dr.Read())
                {
                    ...
                    Content = (ShowCategoryNameInContent) ? Content.Replace("$_db category_name;", Language.GetHandheldLanguageWithoutNullLanguageSuffix(dbdr.dr["category_name"].ToString(), StaticObject.GetCurrentSiteGlobalLanguage())) : Content.Replace("$_db category_name;", null);
        ...
        public string GetCurrentProtectionAttachment(string AttachmentId, string AttachmentPassword)
        {
            ...
            if (dbdr.dr != null && dbdr.dr.HasRows)
            {
                ...
                AttachmentFileTemplate = AttachmentFileTemplate.Replace("$_asp attachment_extension_icon;", System.IO.Path.GetExtension(dbdr.dr["attachment_physical_name"].ToString()).Remove(0, 1));
