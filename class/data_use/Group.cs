...
namespace elanat.DataUse
{
    public class Group : IDisposable
    {
        ...
        public void AddRoleGroup(string GroupId, ListItemCollection RoleGroupListItem)
        {
            DataBaseSocket db = new DataBaseSocket();
        ...
        // Overload
        public void AddRoleGroup(ListItemCollection RoleGroupListItem)
        {
            DataBaseSocket db = new DataBaseSocket();
            foreach (ListItem group in RoleGroupListItem)
                if (group.Selected)
        ...
        public void SetGroupRole(string GroupId, ListItemCollection GroupAccessShowListGroup)
        {
            DataBaseSocket db = new DataBaseSocket();
        ...
        // Overload
        public void SetGroupRole(ListItemCollection GroupAccessShowListGroup)
        {
            DataBaseSocket db = new DataBaseSocket();
            foreach (ListItem item in GroupAccessShowListGroup)
                if (item.Selected)
        ...
        public NameValueCollection RoleBitColumnAccess = new NameValueCollection();
        public void FillCurrentAggregationRole(string GroupId)
        {
            ...
            while (dbdr.dr.Read())
                foreach (ListItem li in lc.RoleBitColumnAccessListItem)
                {
                    if (RoleBitColumnAccess[li.Value] == null)
                        RoleBitColumnAccess.Add(li.Value, dbdr.dr[li.Value].ToString().TrueFalseToZeroOne());
                    else
                    {
                        if (RoleBitColumnAccess[li.Value] ==  dbdr.dr[li.Value].ToString().TrueFalseToZeroOne())
                            continue;

                        RoleBitColumnAccess[li.Value] = "2";
                    }
                }
        ...
        {
            return RoleBitColumnAccess[ColumnName];
        }

        public NameValueCollection GetRoleIdAndNameByGroupId(string GroupId)
        {
            DataBaseSocket db = new DataBaseSocket();
            DataBaseDataReader dbdr = new DataBaseDataReader();
