    ...
    public class ModulePathAccess
    {
        ...
        public string GetModuleDirectoryPathFromPath(string Path)
        {
            ...
            for (int i = 0; i < ModuleDirectoryPathArrayLength; i++)
            {
                ...
                if (File.Exists(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "add_on/module/" + string.Join("/", ModuleDirectoryPathArray) + "/catalog.xml")))
        ...
        public bool IsModuleAccessAddPath(string ModuleDirectoryPath, string Path, string QueryStringValue, string FormValue)
        {
            ...
            CatalogDocument.Load(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "add_on/module/" + ModuleDirectoryPath + "/catalog.xml"));
            ...
            LoadValueCheck:
                try
                {
                    ...
                    if (CheckType == "method")
                    {
                        ...
                        string ReturnValue = ml.Start(HttpContext.Current.Server.MapPath(LoadValueCheck), DllType, DllMethod, ObjectParameters, IsNonPublic);
                        if (ReturnValue == "true")
                        {
                            IsUserAccessToModulePathForEditOwn = true;
                        }
