...
namespace elanat
{
    public class ExtraHelperPathAccessShow
    {
        ...
        public string GetExtraHelperDirectoryPathFromPath(string Path)
        {
            ...
            string ExtraHelperDirectoryPath = "";
            ExtraHelperDirectoryPath = fad.GetCleanDirectoryPathFromPath(Path);
            ...
            for (int i = 0; i < ExtraHelperDirectoryPathArrayLength; i++)
            {
                ...
                if (File.Exists(HttpContext.Current.Server.MapPath(StaticObject.SitePath + "add_on/extra_helper/" + string.Join("/", ExtraHelperDirectoryPathArray) + "/catalog.xml")))
