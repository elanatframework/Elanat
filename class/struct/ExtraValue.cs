            ...
            while (TextStruct.Contains("$_asp "))
            {
                string Variable = TextStruct.Split(new string[] { "$_asp " }, StringSplitOptions.None)[1].Split(';')[0].Trim();
                {
                    ...
                    switch (TmpVariable)
                    {
                        ...
                        case "file_physical_name_without_extension": TextStruct = TextStruct.Replace("$_asp " + Variable + ";", System.IO.Path.GetFileNameWithoutExtension(FilePhysicalName)); break;
                        case "file_extension": TextStruct = TextStruct.Replace("$_asp " + Variable + ";", System.IO.Path.GetExtension(FilePhysicalName)); break;
