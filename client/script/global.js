        ...
        else
        {
            ...
            xmlhttp.onreadystatechange = function ()
            {
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
                {
                    ...
                    CaptchaValueArray.item(i).innerHTML = DivTag.outerHTML;
            ...
            xmlhttp.open("GET", ElanatVariant.SitePath + "action/show_captcha/Default.aspx", false);
            xmlhttp.send();
        }
    }
}
...
function el_ViewFile(FileId, FilePath)
{
    ...
    var FileViewerPath = ElanatVariant.FileViewerPath;
    ...
    xmlhttp.open("GET", ElanatVariant.SitePath + "include/file_viewer/" + FileViewerPath + "/" + FileType + "/" + FileExtensions, false);
    xmlhttp.send();
}
...
function el_InsertEditorTemplateValueToWysiwyg()
{
    ...
    EditorTemplateParentDiv.innerHTML = ElanatVariant.EditorTemplateValue;
...
function el_LoadPageWithAjax(Url, FetchScript)
{
    var xmlhttp = (window.XMLHttpRequest) ? xmlhttp = new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
...
function el_AjaxPostBack(obj, FetchScript, PostbackSectionId, Retrieved)
{
    ...
    var StartChar = (FormAction.indexOf('?') > -1) ? "&" : "?";

    var QueryString = StartChar + el_Serialize(FormElement) + "&" + TagSubmitName + "=" + TagSubmitValue;
    ...
    xmlhttp.send(el_FormDataSerialize(FormElement, TagSubmitName, TagSubmitValue));
}
...
function el_InsertScriptFromText(HtmlSource)
{
    while (HtmlSource.indexOf("<script") > -1 || HtmlSource.indexOf("</script") > -1)
    {
        ...
        while (CurrentScript.indexOf("=") > -1)
        {
            ...
            var ValueIndex = CurrentScript.indexOf(EqualIndex + 1, Block);
...
function el_InsertStyleFromText(HtmlSource)
{
    while (HtmlSource.indexOf("<style") > -1 || HtmlSource.indexOf("</style") > -1)
    {
        ...
        while (CurrentStyle.indexOf("=") > -1)
        {
            ...
            var ValueIndex = CurrentStyle.indexOf(EqualIndex + 1, Block);
...
function el_FormDataSerialize(form, TagSubmitName, TagSubmitValue)
{
